...
const MAX_PAGES_PER_BROWSER = 4;
const pool = [];

function getNextProxy() {
  const proxy = PROXY_POOL[currentProxyIndex];
  currentProxyIndex = (currentProxyIndex + 1) % PROXY_POOL.length;
  console.log(`🌐 Using proxy ${currentProxyIndex}/${PROXY_POOL.length}: ${proxy.host}`);
  return proxy;
}
...
async function getPage(retries = 0) {
  let entry = pool.find((e) => e.pages < MAX_PAGES_PER_BROWSER);
  ...
  if (!entry) {
    if (retries >= 12) throw new Error('Browser pool exhausted');
...
async function releasePage(page, entry) {
  ...
  if (entry && entry.pages > 0) entry.pages--;
}
